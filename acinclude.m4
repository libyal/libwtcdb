...
AC_DEFUN([LIBWTCDB_TEST_ENABLE],
 [AC_ARG_ENABLE(
  [$1],
  [AS_HELP_STRING(
   [--enable-$1],
   [$3 (default is $4)])],
  [ac_cv_libwtcdb_enable_$2=$enableval],
  [ac_cv_libwtcdb_enable_$2=$4])dnl
  AC_CACHE_CHECK(
   [whether to enable $3],
   [ac_cv_libwtcdb_enable_$2],
   [ac_cv_libwtcdb_enable_$2=$4])dnl
 ])

dnl Function to detect whether nl_langinfo supports CODESET
AC_DEFUN([LIBWTCDB_CHECK_FUNC_LANGINFO_CODESET],
 ...
 AS_IF(
  ...
  [AC_CACHE_CHECK(
   ...
   [ac_cv_libwtcdb_langinfo_codeset],
   [AC_LANG_PUSH(C)
   AC_LINK_IFELSE(
    [AC_LANG_PROGRAM(
     [[#include <langinfo.h>]],
     [[char* charset = nl_langinfo( CODESET );]]) ],
    [ac_cv_libwtcdb_langinfo_codeset=yes],
    [ac_cv_libwtcdb_langinfo_codeset=no])
   AC_LANG_POP(C) ]) ],
  [ac_cv_libwtcdb_langinfo_codeset=no])

 AS_IF(
  [test "x$ac_cv_libwtcdb_langinfo_codeset" = xyes],
  [AC_DEFINE(
...
AC_DEFUN([LIBWTCDB_CHECK_FUNC_PRINTF_JD],
 [AC_MSG_CHECKING(
 ...
 AC_LINK_IFELSE(
  [AC_LANG_PROGRAM(
   [[#include <stdio.h>]],
   [[printf( "%jd" ); ]] )],
  [ac_cv_libwtcdb_have_printf_jd=no],
  [ac_cv_libwtcdb_have_printf_jd=yes])

 dnl Second try to see if compilation and linkage with a parameter succeeds
 AS_IF(
  [test "x$ac_cv_libwtcdb_have_printf_jd" = xyes],
  [AC_LINK_IFELSE(
   [AC_LANG_PROGRAM(
    [[#include <sys/types.h>
#include <stdio.h>]],
...
if( ( string[ 0 ] != '1' ) || ( string[ 1 ] != '0' ) ) return( 1 ); ]] )],
    [ac_cv_libwtcdb_have_printf_jd=yes],
    [ac_cv_libwtcdb_have_printf_jd=no],
    [ac_cv_libwtcdb_have_printf_jd=undetermined])
   ])
...
AC_DEFUN([LIBWTCDB_CHECK_FUNC_PRINTF_ZD],
 ...
 AC_LINK_IFELSE(
  [AC_LANG_PROGRAM(
   [[#include <stdio.h>]],
   [[printf( "%zd" ); ]] )],
  [ac_cv_libwtcdb_have_printf_zd=no],
  [ac_cv_libwtcdb_have_printf_zd=yes])

 dnl Second try to see if compilation and linkage with a parameter succeeds
 AS_IF(
  [test "x$ac_cv_libwtcdb_have_printf_zd" = xyes],
  [AC_LINK_IFELSE(
   [AC_LANG_PROGRAM(
    [[#include <sys/types.h>
#include <stdio.h>]],
...
if( ( string[ 0 ] != '1' ) || ( string[ 1 ] != '0' ) ) return( 1 ); ]] )],
    [ac_cv_libwtcdb_have_printf_zd=yes],
    [ac_cv_libwtcdb_have_printf_zd=no],
    [ac_cv_libwtcdb_have_printf_zd=undetermined])
   ])
 ...
 AS_IF(
  ...
  AC_LINK_IFELSE(
   AC_LANG_PROGRAM(
    ...
    ac_cv_libwtcdb_ctime_r_size=yes],
    [ac_cv_libwtcdb_ctime_r_size=no])

  AS_IF(
   [test "x$ac_cv_libwtcdb_ctime_r_size" = xno],
   [AC_LINK_IFELSE(
    AC_LANG_PROGRAM(
     [[#include <time.h>]],
     [[ctime_r( NULL, NULL )]]),
    [AC_MSG_RESULT(
     [with two arguments])
    ac_cv_libwtcdb_ctime_r_posix=yes],
    [ac_cv_libwtcdb_ctime_r_posix=no])
   ])

  AS_IF(
   [test "x$ac_cv_libwtcdb_ctime_r_posix" = xno],
  ...
  AS_IF(
   [test "x$ac_cv_libwtcdb_ctime_r_size" = xno && test "x$ac_cv_libwtcdb_ctime_r_posix" = xno],
   [AC_MSG_WARN(
    [unknown])
  ...
  AS_IF(
   [test "x$ac_cv_libwtcdb_ctime_r_size" = xyes],
   [AC_DEFINE(
    [HAVE_CTIME_R_SIZE],
    [1],
...
AC_DEFUN([LIBWTCDB_CHECK_FUNC_MKDIR],
...
#include <sys/types.h>]],
    ...
    ac_cv_libwtcdb_mkdir_mode=yes],
    [ac_cv_libwtcdb_mkdir_mode=no])

  AS_IF(
   [test "x$ac_cv_libwtcdb_mkdir_mode" = xno],
   [AC_LINK_IFELSE(
    AC_LANG_PROGRAM(
     [[#include <io.h>]],
     [[mkdir( "" )]]),
    ...
    ac_cv_libwtcdb_mkdir=yes],
    [ac_cv_libwtcdb_mkdir=no])
   ])
  ...
  AS_IF(
   [test "x$ac_cv_libwtcdb_mkdir_mode" = xno && test "x$ac_cv_libwtcdb_mkdir" = xno],
   [AC_MSG_WARN(
    [unknown])
   ac_cv_func_mkdir=no])
  ...
  AS_IF(
   [test "x$ac_cv_libwtcdb_mkdir_mode" = xyes],
   [AC_DEFINE(
    [HAVE_MKDIR_MODE],
    [1],
...
AC_DEFUN([LIBWTCDB_CHECK_LIBUNA],
 [AC_CHECK_HEADERS([libuna.h])

 AS_IF(
  [test "x$ac_cv_header_libuna_h" = xno],
  [ac_libwtcdb_have_libuna=no],
  [ac_libwtcdb_have_libuna=yes
  AC_CHECK_LIB(
   una,
   libuna_get_version,
   [],
...
AC_DEFUN([LIBWTCDB_CHECK_LIBBFIO],
 [AC_CHECK_HEADERS([libbfio.h])

 AS_IF(
  [test "x$ac_cv_header_libbfio_h" = xno],
  [ac_libwtcdb_have_libbfio=no],
  [ac_libwtcdb_have_libbfio=yes
  AC_CHECK_LIB(
   bfio,
   libbfio_get_version,
   [],
  ...
  AS_IF(
   [test "x$ac_cv_libwtcdb_enable_wide_character_type" != xno],
   [AC_CHECK_LIB(
    bfio,
