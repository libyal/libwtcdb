...
LIBRARY_TESTS="cache_entry crc error file_header io_handle item item_value notify";
...
run_test()
{
	...
	local TEST_EXECUTABLE="./wtcdb_test_${TEST_NAME}";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libwtcdb" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
